...
namespace FxCommonLib.Utils {
    ...
    public class FileUtil {
        ...
        /// <summary>
        /// 指定したテキストファイルの文字コードを判定します。
        /// </summary>
        /// <param name="path"></param>
        public static System.Text.Encoding GetTextEncoding(string path) {
            byte[] bytes = null;
            ...
            string str = null;
            var charCode = ReadJEnc.JP.GetEncoding(bytes, bytes.Length, out str);
            if (charCode == CharCode.SJIS) {
                //CP932
                return Encoding.GetEncoding("Shift_JIS");
            } else if (charCode == CharCode.UTF32B) {
                //UTF32 BE(BOMあり)
                return new System.Text.UTF32Encoding(true, true);
            } else if (charCode == CharCode.UTF32) {
                //UTF32 LE(BOMあり)
                return new System.Text.UTF32Encoding(false, true);
            } else if (charCode == CharCode.UTF16B) {
                //UTF16 BE(BOMあり)
                return new System.Text.UnicodeEncoding(true, true);
            } else if (charCode == CharCode.UTF16) {
                //UTF16 LE(BOMあり)
                return new System.Text.UnicodeEncoding(false, true);
            } else if (charCode == CharCode.UTF8) {
                //UTF8(BOMあり)
